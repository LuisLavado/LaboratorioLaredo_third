...
<body>
    <div class="header">
        <h1>{{ $title ?? 'Reporte de Exámenes' }}</h1>
        <div class="subtitle">Análisis Detallado de Exámenes</div>
        <div class="subtitle">
            Período: {{ $startDate ?? 'N/A' }} - {{ $endDate ?? 'N/A' }}
        </div>
        @if(isset($generatedBy))
        <div class="subtitle">Generado por xd: {{ $generatedBy }}</div>
        @endif
    ...
    <div class="section">
        <h2 class="section-title">Estado de Exámenes</h2>
        <div class="stats-grid">
            <div class="stat-card">
    ...
    <div class="section">
        <h2 class="section-title">Exámenes Solicitados</h2>
        <table>
            <thead>
                <tr>
                    <th>Código</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $exam->categoria ?? 'General' }}</td>
                    <td class="text-center">{{ $exam->count ?? 0 }}</td>
                    <td class="text-center">{{ $exam->percentage ?? 0 }}%</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="footer">
        <p>Reporte de Exámenes del Sistema de Laboratorio</p>
