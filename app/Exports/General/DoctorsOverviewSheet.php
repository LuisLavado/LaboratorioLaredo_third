...
class DoctorsOverviewSheet implements FromArray, WithHeadings, WithTitle, WithStyles, WithColumnWidths
{
      ...
      public function array(): array
    {
        ...
        if (empty($this->doctors) || (is_array($this->doctors) && empty($this->doctors['doctorStats']))) {
            $rows[] = [
        ...
        $doctorsList = is_array($this->doctors) && isset($this->doctors['doctorStats']) 
            ? $this->doctors['doctorStats'] 
            : $this->doctors;

        foreach ($doctorsList as $doctor) {
            // Convert to object if it's an array
            $doctor = is_array($doctor) ? (object)$doctor : $doctor;
